...
/pause - Səsli söhbətdə oxunan musiqini dayandırın.

/resume - Səsli söhbətdə dayandırılmış musiqini davam etdirin.

/skip - Səsli söhbətdə cari ifa olunan musiqini keçin

/stop - Musiqini Bitir.

/queue - Növbə siyahısını yoxlayın..
"""
...
async def admins(_, message: Message):
    ...
    if not await is_active_chat(message.chat.id):
        return await message.reply_text(
            "Səsli çatda heç nə oxunmur. Aktiv Səsli Söhbət Tapılmadı"
        )
    ...
    if message.command[0][1] == "t" or message.command[0][1] == "n":
        ...
        await message.reply_text(
            f"🎧 Voicechat Bitdi/Dayandırıldı {message.from_user.mention}!"
        )
    if message.command[0][1] == "k":
        ...
        if Queues.is_empty(chat_id):
            ...
            await remove_active_video_chat(chat_id)
            await message.reply_text(
                "Növbədə musiqi __yoxdur__ \n\nSəsli çatı tərk edir"
            )
            await stop_stream(chat_id)
            return
        else:
            ...
            if str(finxx) == "raw":
                ...
                final_output = await message.reply_photo(
                    ...
                    caption=f"<b>__Səsli Söhbət Buraxıldı__</b>\n\n🎥<b>__Oxumağa Başladı:__</b> {title} \n⏳<b>__Vaxd:__</b> {duration_min} \n👤<b>__Tələb Etdi:__ </b> {mention}",
                )
                await start_timer(
                    videoid,
                    duration_min,
                ...
                if int(quality) == 1080:
                    try:
                        ...
                        return await mystic.edit(
                            f"Video axını dəyişdirərkən xəta baş verdi.\n\nnMümkün Səbəb:- {e}"
                        )
                    ...
                    final_output = await message.reply_photo(
                        ...
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=(
                            f"<b>__Keçid Edilmiş Video Söhbət__</b>\n\n👤**__Tələb Etdi:__** {mention}"
                        ),
                    )
                    await mystic.delete()
                else:
                    ...
                    final_output = await message.reply_photo(
                        ...
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=(
                            f"<b>__Keçid Edilmiş Video Söhbət__</b>\n\n🎥<b>__Video Oynamağa Başladı:__ </b> [{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n👤**__Tələb Etdi:__** {mention}"
                        ),
                    )
                    await mystic.delete()
                    os.remove(thumb)
                ...
                mystic = await message.reply_text(
                    f"**{MUSIC_BOT_NAME} Pleylist Funksiyası**\n\n__Pleylistdən Növbəti Musiqi Yüklənir....__"
                )
                (
                    title,
                    duration_min,
