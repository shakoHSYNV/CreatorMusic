...
async def play(_, message: Message):
    ...
    if audio:
        mystic = await message.reply_text(
            "🔄 Audio emal olunur... Zəhnət olmada gözləyin!"
        )
        try:
        ...
        if not limit:
            return await message.reply_text(
                "**Video Zənglər üçün Limit Müəyyən edilməyib**\n\nBot tərəfindən icazə verilən Maksimum Video Zənglərin Sayı üçün Limit təyin edin /set_video_limit [Yalnız Sudo İstifadəçiləri]"
            )
        ...
        mystic = await message.reply_text(
            "🔄 Video emal olunur... Zəhnət olmada gözləyin!"
        )
        try:
            read = db_mem[message.chat.id]["live_check"]
            if read:
        ...
        mystic = await message.reply_text("🔄 URL emal olunur... Zəhmət olmasa gözləyin!")
        ...
        if len(message.command) < 2:
            ...
            await message.reply_photo(
                photo="Utils/Playlist.gif",
                caption=(
                    "**İstifadəsi:** /play [Musiqi Adı və ya Youtube Linki və ya Audioya Cavab]\n\nPleylistləri qoşmaq istəyirsinizsə! Aşağıdan birini seçin"
                ),
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            return
...
async def Music_Stream(_, CallbackQuery):
    ...
    if str(duration) == "None":
        ...
        return await CallbackQuery.edit_message_text(
            "**Canlı Yayım Aşkarlandı**\n\nCanlı yayım oynamaq istəyirsiniz? Bu, cari ifa olunan musiqiləri (əgər varsa) dayandıracaq və canlı video yayımına başlayacaq.",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    if CallbackQuery.from_user.id != int(user_id):
        return await CallbackQuery.answer(
            "Bu sizin üçün deyil! Zəhmət olmasa Öz Mahnınızı axtarın.",
            show_alert=True,
        )
